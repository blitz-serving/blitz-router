...
def eval_all():
    ...
    config = load_config("scripts/batch/config.yaml")
    ...
    if inter_node_server:
        ...
        set_args("max_prefill_num", config["default_settings"]["max_prefill_num"])
        set_args("max_decode_num", config["default_settings"]["max_decode_num"])
    ...
    for memory_available in ["Full"]:
        for model_name, model_config in model_configs.items():
            ...
            init_prefill_num = config["default_settings"]["init_prefill_num"]
            init_decode_num = config["default_settings"]["init_decode_num"]
            num_gpus_per_node = config["default_settings"]["num_gpus_per_node"]
            ...
            for dataset_name in model_config["traces"][memory_available]:
                ...
                for feature_tag, feature in feature_configs.items():
                    ...
                    relative_workdir = f"{relative_log_home}/{task_start_time}-{node_level}-eval-{memory_available}/{model_name}/{dataset_name}/{feature_tag}"
                    absolute_workdir = f"{absolute_log_home}/{task_start_time}-{node_level}-eval-{memory_available}/{model_name}/{dataset_name}/{feature_tag}"
